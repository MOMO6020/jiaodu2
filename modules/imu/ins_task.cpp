...
{
    float output = TempCtrl.PIDCalculate(BMI088.Temperature, RefTemp);
    IMUPWMSet(float_constrain(float_rounding(output), 0, UINT32_MAX));
}
...
attitude_t* INS_Init(void)
{
    ...
    // imu heat init
    TempCtrl =
        PIDController(PIDController::PID_Config{1000, 20, 0, 2000, 0.1, PIDImprovement::PID_Integral_Limit, 300});

    // noise of accel is relatively big and of high freq,thus lpf is used
    INS.AccelLPF = 0.0085;
    STM32TimeDWT::GetDelta(INS_DWT_Count);  // 计时器初始化
    LOGINFO("IMU", "IMU Initialized.");
...
void INS_Task(void)
{
    ...
    dt = STM32TimeDWT::GetDelta(INS_DWT_Count) / 1000.0f;
    if (dt < 0.001f)
        dt = 0.001f;
    t += dt;
